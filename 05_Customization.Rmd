...
Note the inline vs. console stuff mostly just has to do with the actual .Rmd file, not the output, so we're going to ignore it[^inline]. Within the options you can apply some default settings to images, code, and more. 
...
mgcv::gamSim(verbose = F) %>% 
  ...
  plot_ly(x=~x2, y=~y) %>% 
  add_lines(color=I(palettes$orange$complementary[2]), showlegend=F, opacity=.5) %>% 
  add_lines(y=~pred, color=I(palettes$orange$orange), showlegend=F) %>% 
  theme_plotly()
```
...
Recall the style section in some of the HTML examples above. For example, the part `style='color:#66023C; font-size:150%; font-weight:600'` changed the font.  <span class="marginnote">CSS stands for cascading style sheets</span>It's actually CSS, and if we need to do the same thing each time, we can take an alternative approach to creating a style that would apply to all objects of the same class throughout the document.

The first step is to create a `*.css` file that your R Markdown document can refer. Let's say we want to make every link <span style='color:dodgerblue'>dodgerblue</span>.  Links in HTML are tagged with the letter **a**, and to insert a link with HTML you can do something like:
...
You could make images always display at 50% width by default.
...
How much you want to get into customization is up to you.  Using the developer tools of any web browser allows you to inspect what anyone else has done as far as styling with CSS.  Here is an example of Chrome Developer Tools, which you can acess through its menus.
...
[^inline]: I haven't yet figured out the utility in having figures and output breaking up the flow of code and text along with everything else going on. Just my opinion though.